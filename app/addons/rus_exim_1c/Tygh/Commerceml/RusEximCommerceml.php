...
class RusEximCommerceml
{
    ...
    /** @var string */
    public static $dir_name_date_format = 'dmY';
    ...
    public function __construct(Connection $db, Logs $log, $path_commerceml)
    {
        ...
        $this->path_file = 'exim/1C_' . date(self::$dir_name_date_format) . '/';
            ...
            foreach ($data_features -> {$cml['property']} as $_feature) {
                ...
                if (!empty($_feature -> {$cml['variants_values']})) {
                    ...
                    foreach ($_feature_data as $_variant) {
                        ...
                        $f_variants[(string) $_variant -> {$cml['id_value']}]['commerceml'] = true;
                    }
                }
    ...
    {
        $feature_type = fn_get_product_feature_type_by_feature_id($external_id);
    ...
    public function dataProductFeatures($data_product, $product, $import_params)
    {
        ...
        if ($this->s_commerceml['exim_1c_used_brand'] === 'field_brand') {
            if (isset($data_product -> {$cml['manufacturer']})) {
                ...
                $variant_data['lang_code'] = $import_params['lang_code'];
                $_variant_data = $variant_id = $this->db->getRow(
                    'SELECT pfv.variant_id, pfvd.variant'
                    . ' FROM ?:product_feature_variants AS pfv'
                    ...
                    $variant_data['lang_code'],
                    $variant_data['feature_id'],
                    (string) $data_product->{$cml['manufacturer']}->{$cml['id']}
                );

                $variant_id  = isset($_variant_data['variant_id']) ? $_variant_data['variant_id'] : null;
                $old_variant = isset($_variant_data['variant']) ? $_variant_data['variant'] : '';

                $variant = (string) $data_product->{$cml['manufacturer']}->{$cml['name']};
                if (empty($variant_id)) {
                    $variant_data['variant_id'] = fn_add_feature_variant($variant_data['feature_id'], ['variant' => $variant]);
                    $this->db->query(
                        'UPDATE ?:product_feature_variants SET external_id = ?s WHERE variant_id = ?i',
                        (string) $data_product->{$cml['manufacturer']}->{$cml['id']},
                ...
                $product['features'][$variant_data['feature_id']]['feature_type'] = 'E';
            }
        }
    ...
    public function isFileProductImage($filename)
    {
        ...
        if (is_array($file_array)) {
            $type = mb_strtolower(array_pop($file_array));
            if (in_array($type, ImageHelper::getSupportedFormats())) {
                return true;
            }
        }
        ...
        if (!empty($order_data['subtotal']) && (!empty($order_data['discount']) || !empty($order_data['subtotal_discount']))) {
            ...
            if (!empty($order_data['discount'])) {
                foreach ($order_data['products'] as $product) {
                    $o_subtotal = $o_subtotal + ($product['price'] * $product['amount']);
                }
            }
