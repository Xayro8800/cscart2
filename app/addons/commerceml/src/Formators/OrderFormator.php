...
class OrderFormator
{
    ...
    {
        $order_currency = $this->default_currency;
    ...
    private function formCustomerData(array $order_data)
    {
        ...
        $contractor_xml = [
            ...
            SimpleXmlElement::findAlias('role') => SimpleXmlElement::findAlias('buyer'),
        ...
        /** @var array $order_data['cml_profile_fields'] */
        $profile_fields = $this->formProfileFields($order_data['cml_profile_fields']);

        return [
            SimpleXmlElement::findAlias('contractors') => [
                SimpleXmlElement::findAlias('contractor') => array_merge($profile_fields, $contractor_xml)
            ]
        ];
    }
    ...
    private function formDiscountData(array $order_data)
    {
        ...
        if (!empty($order_data['discount'])) {
            ...
            foreach ((array) $order_data['products'] as $product) {
                $order_subtotal = $order_subtotal + ((float) $product['price'] * (int) $product['amount']);
            }
        }
     ...
     *
     * @param array<string, string> $fields Profile fields
     *
     * @return array<string, string>
     */
    private function formProfileFields(array $fields)
    {
        $result = [];

        foreach ($fields as $name => $value) {
            $result[mb_convert_case($name, MB_CASE_TITLE)] = $value;
        }
